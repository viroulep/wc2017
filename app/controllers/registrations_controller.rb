class RegistrationsController < ApplicationController
  ...
  def printable_schedules
    @registrations = Registration.includes([registration_detail: [], scramble_events: [], user: [], groups: [:round], staff_registrations_groups: { group: [:round] }, staff_teams_groups: { group: [:round] }]).order(:id)
    @side_event = ["333mbf", "444bf", "555bf"]
    length = 500
    offset = 0
    case params[:type]
    when "staff"
      @registrations = @registrations.staff_available.shuffle
    else
      offset = params[:off]&.to_i || 0
      @registrations = @registrations.accepted.limit(length).offset(offset).reject { |r| r.staff? }
    end
  end
